...
def extract_price(v, fb=0):
    if v is None: return fb
    if isinstance(v, (int,float)): return v if v>0 else fb
    if isinstance(v, str):
        try: return float(v.replace("₹","").replace(",","").strip())
        except: return fb
    if isinstance(v, dict):
        for k in ["value","selling_price","price","mrp"]:
            if v.get(k): return extract_price(v[k], fb)
    return fb
...
# ============ REAL OTPLESS OTP ============
def send_otp_otpless(phone, user_id):
    ...
    try:
        ...
        if state_resp.status_code != 200: return {"ok":False,"message":"State failed"}
        ...
        if intent_resp.status_code != 200: return {"ok":False,"message":"Intent failed"}
        
        data = intent_resp.json()
        leap = data.get("quantumLeap", {})
        if not leap.get("uid") or not leap.get("channelAuthToken"):
            return {"ok":False,"message":"No leap"}
        
        session = {"state":state,"uid":leap["uid"],"token":leap["channelAuthToken"],"as_id":leap.get("asId",""),"ts_id":ts_id,"in_id":in_id,"instance_id":uuid.uuid4().hex,"phone":phone,"user_id":user_id}
...
def verify_otp_otpless(phone, otp):
    ...
    if not session: return {"ok":False,"message":"OTP pehle bhejo"}
    ...
    try:
        ...
        if verify_resp.status_code != 200: return {"ok":False,"message":"OTP verify failed"}
        ...
        if not token or not id_token: return {"ok":False,"message":"No token"}
        ...
        if login_resp.status_code != 200: return {"ok":False,"message":"Meesho login failed"}
        
        data = login_resp.json()
        user = data.get("user", {})
        xo_data = data.get("xoox", {})
        
        account = {"phone": user.get("phone", phone), "user_id": user.get("user_id", ""), "xo": xo_data.get("xo", ""), "is_new": user.get("new", False)}
...
class APIHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed.path == '/api/search':
            ...
            orders = [o for o in load_orders() if o.get('phone')==phone]
    ...
    def do_POST(self):
        ...
        if parsed.path == '/api/send_otp':
            ...
            self._json(send_otp_otpless(phone, user_id))
        elif parsed.path == '/api/verify_otp':
            self._json(verify_otp_otpless(body.get('phone',''), body.get('otp','')))
